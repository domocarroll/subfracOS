...
subfrac-refresh-config uwsm/default
subfrac-refresh-config uwsm/env
subfrac-state set reboot-required
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, subfrac-launch-editor' ~/.config/hypr/bindings.conf
fi
