...
clear
echo
tte -i ~/.local/share/subfrac/logo.txt --canvas-width 0 --anchor-text c --frame-rate 920 laseretch
echo

# Display installation time if available
if [[ -f $SUBFRAC_INSTALL_LOG_FILE ]] && grep -q "Total:" "$SUBFRAC_INSTALL_LOG_FILE" 2>/dev/null; then
  echo
  TOTAL_TIME=$(tail -n 20 "$SUBFRAC_INSTALL_LOG_FILE" | grep "^Total:" | sed 's/^Total:[[:space:]]*//')
...
if gum confirm --padding "0 0 0 $((PADDING_LEFT + 32))" --show-help=false --default --affirmative "Reboot Now" --negative "" ""; then
  ...
  if [[ -n "${SUBFRAC_CHROOT_INSTALL:-}" ]]; then
