start_log_output() {
  ...
  (
    ...
    while true; do
      ...
      mapfile -t current_lines < <(tail -n $log_lines "$SUBFRAC_INSTALL_LOG_FILE" 2>/dev/null)
...
start_install_log() {
  sudo touch "$SUBFRAC_INSTALL_LOG_FILE"
  sudo chmod 666 "$SUBFRAC_INSTALL_LOG_FILE"
  ...
  if [[ -n ${SUBFRAC_INSTALL_LOG_FILE:-} ]]; then
    ...
    if [ -f "/var/log/archinstall/install.log" ]; then
      ...
      if [ -n "$ARCHINSTALL_START" ] && [ -n "$ARCHINSTALL_END" ]; then
        ...
        echo "Archinstall: ${ARCH_MINS}m ${ARCH_SECS}s" >>"$SUBFRAC_INSTALL_LOG_FILE"
      fi
    fi

    if [ -n "$SUBFRAC_START_TIME" ]; then
      SUBFRAC_START_EPOCH=$(date -d "$SUBFRAC_START_TIME" +%s)
      SUBFRAC_END_EPOCH=$(date -d "$SUBFRAC_END_TIME" +%s)
      SUBFRAC_DURATION=$((SUBFRAC_END_EPOCH - SUBFRAC_START_EPOCH))
      ...
      if [ -n "$ARCH_DURATION" ]; then
        ...
        echo "Total:       ${TOTAL_MINS}m ${TOTAL_SECS}s" >>"$SUBFRAC_INSTALL_LOG_FILE"
      fi
    fi
    echo "=================================" >>"$SUBFRAC_INSTALL_LOG_FILE"

    echo "Rebooting system..." >>"$SUBFRAC_INSTALL_LOG_FILE"
  fi
}
...
run_logged() {
  ...
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting: $script" >>"$SUBFRAC_INSTALL_LOG_FILE"

  # Use bash -c to create a clean subshell
  bash -c "source '$script'" </dev/null >>"$SUBFRAC_INSTALL_LOG_FILE" 2>&1

  local exit_code=$?

  if [ $exit_code -eq 0 ]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Completed: $script" >>"$SUBFRAC_INSTALL_LOG_FILE"
    unset CURRENT_SCRIPT
  else
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Failed: $script (exit code: $exit_code)" >>"$SUBFRAC_INSTALL_LOG_FILE"
  fi
