...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "subfracOS failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "subfracOS failed to open a window: {e:?}. See https://subfracture.studio for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "studio.subfracture.Oops";
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("subfracOS failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://subfracture.studio for troubleshooting steps."
                            )
                            .as_str(),
                        ))
