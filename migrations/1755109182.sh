...
# Users can still opt-in to Cloudflare DNS using: subfrac-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-subfrac-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-subfrac-dns.network
  ...
  echo "To use Cloudflare DNS, run: subfrac-setup-dns Cloudflare"
fi